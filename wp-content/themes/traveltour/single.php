...
// switch_to_locale('th');
	...
	// if( get_the_ID() == 1314 ){
	// 	$o = get_option('icl_sitepress_settings', array());

	// 	if( !empty($o['translation-management']['custom_fields_unlocked_config']['tourmaster-tour-option']) ){
	// 		$o['translation-management']['custom_fields_unlocked_config']['tourmaster-tour-option'] = 0;
	// 		update_option('icl_sitepress_settings', $o);
	// 	}
	// }
...
// esc_html_e('Post Comment', 'traveltour');
	...
	while( have_posts() ){ the_post();
		...
		// tag 
		$blog_style = traveltour_get_option('general', 'single-blog-style', '');
		echo '<div class="traveltour-single-tags clearfix" >';
		...
		if( traveltour_get_option('general', 'blog-social-share', 'enable') == 'enable' ){
			if( class_exists('gdlr_core_pb_element_social_share') ){
				...
				echo gdlr_core_pb_element_social_share::get_content(array(
					...
					'facebook-access-token'=>traveltour_get_option('general', 'blog-facebook-access-token', 'enable'),
